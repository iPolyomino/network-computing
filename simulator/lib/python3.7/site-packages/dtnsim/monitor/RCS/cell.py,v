...
1.3
date	2018.10.15.14.09.14;	author ohsaki;	state Exp;
branches;
next	1.2;

1.2
date	2018.10.15.13.20.27;	author ohsaki;	state Exp;
branches;
next	1.1;

1.1
date	2018.05.27.08.33.07;	author ohsaki;	state Exp;
branches;
next	;
...
@d4 1
a4 1
# Copyright (c) 2011-2016, Hiroyuki Ohsaki.
d10 3
a12 1
import math, random
a14 1
...
a54 2
        for e in graph.edges():
            print('define - link v{e[0]} v{e[1]} 1 c_edge')
d57 9
d67 1
d69 1
a69 1
...
d72 1
a72 1
            r = to_geometry(agent.range)
a76 9
            # display cut-through links among wired agents
            if 'Wired' in type(agent).__name__:
                id = agent.id
                for friend in agent.friends:
                    friend_id = friend.id
                    if friend_id > id:
                        continue
                    print(f'define - link agent{id} agent{friend_id} 2 green')
...
a84 8
    def change_agent_status(self, agent):
        id = agent.id
        color = 'c_sus'
        if len(agent.received) > 0 or len(agent.receive_queue) > 0:
            color = 'c_inf'
        print(f'color agent{id} {color}')
        print(f'color agentr{id} {color}_range')
...
d86 1
a86 1
        tx_total, rx_total, dup_total, uniq_recvd = 0, 0, 0, 0
d99 2
@
